...
public class GetWatchedVideosbyYourFriends5305 {
    //数据没必要变成map，直接用就好
    public List<String> watchedVideosByFriends(List<List<String>> watchedVideos, int[][] friends, int id, int level) {
        Set<Integer> visitedPersonId = new HashSet<>();
        Queue<Integer> queue = new LinkedList<>();
        queue.add(id);
        visitedPersonId.add(id);
        for (int i = 0; i < level; ++i) {
            ...
            for (int j = 0; j < n; ++j) {
                ...
                for (int friendId: friends[curId]) {
                    if (visitedPersonId.add(friendId)) {
                        next.add(friendId);
                    }
                }
            }
        ...
        //get queue of ids of level k
        //map movies name and cnt
        ...
        Collections.sort(list, (a,b) -> a.getValue() == b.getValue() ? a.getKey().compareTo(b.getKey()) : a.getValue() - b.getValue());
