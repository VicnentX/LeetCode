...
public class NumberofOrdersintheBacklog1801 {
    public int getNumberOfBacklogOrders(int[][] orders) {
        ...
        for (int[] order: orders) {
            ...
            if (orderType == 0) {
                while (amount > 0 && sellOrders.size() > 0 && sellOrders.peek()[0] <= price) {
                    int deltaAmount = Math.min(amount, sellOrders.peek()[1]);
                    sellOrders.peek()[1] -= deltaAmount;
                    amount -= deltaAmount;
                ...
                while (amount > 0 && buyOrders.size() > 0 && buyOrders.peek()[0] >= price) {
                    int deltaAmount = Math.min(amount, buyOrders.peek()[1]);
                    buyOrders.peek()[1] -= deltaAmount;
                    amount -= deltaAmount;
